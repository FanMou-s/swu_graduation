...
注意:1.在使用数据前请浏览以确认所有字段格式无误，如某些文献的作者字段可能会多出换行符未处理掉，将导致转换成xls文件时多出空白行
    2.不含关键字字段和分 类字段的文献很可能是内容页没有成功抓取的，这个bug以后改进
...
# 打开存储内容的文件
fileName = config.get("outputPath") + "data-" + config.get("txt_1_value1", "search").decode(
    "utf8") + "-" + time.strftime("%Y%m%d%H%M%S") + ".txt"
...
print("################START(" + time.strftime("%Y-%m-%d %H:%M:%S") + ")列表页总数:" + str(
    totalListPages) + "################")
for i in range(startPage, totalListPages + 1):
    ...
    if i % config.get("restEvery") == 0:
        print("REST(" + time.strftime("%Y-%m-%d %H:%M:%S") + "):又抓取了" + str(config.get("restEvery")) + "页了，休息" + str(
            config.get("restPeriod")) + "秒吧～")
